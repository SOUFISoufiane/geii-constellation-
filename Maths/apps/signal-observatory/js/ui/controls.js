...
import { algebraState, OPS, getAlgebraEligibleSignals, resetAlgebraCache } from '../plots/algebra.js';
import { gameState, setDirection, resetScore, newQuestion } from '../plots/game.js';
...
export function initControls(state, onChange) {
    ...
    const algebraGroup = document.getElementById('algebra-picker-group');
    const gameGroup = document.getElementById('game-controls-group');
    const toggleModeGroups = () => {
        if (algebraGroup) algebraGroup.style.display = (state.activeCombo === 'algebra') ? 'flex' : 'none';
        if (gameGroup)    gameGroup.style.display    = (state.activeCombo === 'game')    ? 'flex' : 'none';
    };
    if (combo) {
        combo.value = state.activeCombo || 'parseval';
        toggleModeGroups();
        combo.addEventListener('change', e => {
            state.activeCombo = e.target.value;
            toggleModeGroups();
            // Generate a fresh question whenever we enter game mode
            if (state.activeCombo === 'game') newQuestion();
            onChange();
        });
    }

    // Algebra: signal-B picker
    const sigBSel = document.getElementById('algebra-sig-b');
    if (sigBSel) {
        sigBSel.innerHTML = getAlgebraEligibleSignals()
            .map(s => `<option value="${s.id}">${s.name}</option>`).join('');
        sigBSel.value = algebraState.sigBId;
        sigBSel.addEventListener('change', e => {
            algebraState.sigBId = e.target.value;
            resetAlgebraCache();
            onChange();
        });
    }

    // Algebra: binary-op picker
    const opSel = document.getElementById('algebra-op');
    if (opSel) {
        opSel.innerHTML = Object.entries(OPS)
            .map(([key, meta]) => `<option value="${key}">${meta.label}</option>`).join('');
        opSel.value = algebraState.op;
        opSel.addEventListener('change', e => {
            algebraState.op = e.target.value;
            resetAlgebraCache();
            onChange();
        });
    }

    // Game: direction picker
    const gameDirSel = document.getElementById('game-direction');
    if (gameDirSel) {
        gameDirSel.value = gameState.direction;
        gameDirSel.addEventListener('change', e => {
            setDirection(e.target.value);
            onChange();
        });
    }

    // Game: reset-score button
    const gameResetBtn = document.getElementById('game-reset-score');
    if (gameResetBtn) {

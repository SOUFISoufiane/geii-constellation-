...
export function renderFusion(mode, signal, state, computed) {
    switch (mode) {
        ...
        case 'winding':   return renderWinding3D(signal, state, computed);
...
// ─── WINDING 3D ────────────────────────────────────────────────────
function renderWinding3D(signal, state, computed) {
    const params = state.params || {};
    ...
    for (let i = 0; i < tRange.length; i++) {
        const t = tRange[i];
        const s = (timeData && timeData[i] !== undefined) ? timeData[i] : signal.calcTime(t, params);
        x.push(t);
    ...
    // Trace 0: The base path (solid if player tab is closed, faint if open)
    // Trace 1: The active path (starts empty, gets filled by playheads.js when playing)
    const baseOpacity = state.playerActive ? 0.2 : 1.0;
    
    Plotly.react('plot-fusion', [
        { x, y, z, type: 'scatter3d', mode: 'lines', opacity: baseOpacity, line: { color: PALETTE.purple, width: state.playerActive ? 2 : 4 }, name: 'Guide', hoverinfo: 'skip' },
        { x: [], y: [], z: [], type: 'scatter3d', mode: 'lines', opacity: 1.0, line: { color: PALETTE.gold, width: 4 }, name: 'Active' }
    ], layout, PLOTLY_CONFIG);
}

...
import { getContinuousWindow } from '../math/utils.js';
...
export function getSampledPoints(signal, params, tStart, tEnd) {
    ...
    const points = { x: [], y: [] };
    const windowType = state.fftWindow || 'rectangular';
    for (let t = tStart; t <= tEnd + 1e-9; t += Ts) {
        const frac = (t - tStart) / (tEnd - tStart);
        const winVal = getContinuousWindow(windowType, frac);
        points.x.push(t);
        points.y.push(signal.calcTime(t, params) * winVal);
    }
    return points;
}
